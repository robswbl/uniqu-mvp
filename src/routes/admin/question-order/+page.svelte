    ...
    try {
      ...
      if (data && data.length) {
        ...
        steps = defaultSteps.map(step => {
          const dbRow = data.find((row: { step_id: string; order: string[] }) => row.step_id === step.id);
          if (dbRow) {
            // Map question IDs from DB to question objects
            const orderedQuestions = dbRow.order.map((qid: string) => getQuestionById(step.id, qid)).filter((q: { id: string; label: string } | undefined): q is { id: string; label: string } => Boolean(q));
...
<div class="min-h-screen bg-gray-50 p-8">
  <div class="max-w-3xl mx-auto">
    <h1 class="text-3xl font-bold mb-8">Admin: Question Order</h1>
    ...
    {:else}
        ...
        <div class="mb-8 bg-white rounded-xl shadow p-6">
          ...
          <ul>
            {#each step.questions as question, questionIdx}
              <li
                class="mb-2 p-3 bg-gray-100 rounded cursor-move {draggingStepIdx === stepIdx && draggingQuestionIdx === questionIdx ? 'opacity-50 ring-2 ring-indigo-400' : ''}"
                draggable="true"
      ...
      {/each}
      <button class="px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700" on:click={saveOrder} disabled={loading}>
        Save Order
      </button>
    {/if}
  </div>
