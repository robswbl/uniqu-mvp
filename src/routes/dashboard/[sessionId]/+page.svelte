<script lang="ts">
	...
	function getAchievementBadge() {
		if (completionPercentage >= 100)
			return { text: 'Profile Complete!', color: 'bg-green-500', icon: '🏆' };
		if (completionPercentage >= 80)
			return { text: 'Almost There!', color: 'bg-blue-500', icon: '🎯' };
		if (completionPercentage >= 60)
			return { text: 'Great Progress!', color: 'bg-purple-500', icon: '⭐' };
		if (completionPercentage >= 40)
			return { text: 'Getting Started', color: 'bg-orange-500', icon: '🌱' };
	...
	onMount(async () => {
		try {
			...
			const { data: session, error: sessionError } = await supabase
				...
				.select(
					'user_id, status, cv_text, ikigai_love, ikigai_good_at, ikigai_care_about, ikigai_inspires, ikigai_want_to_be, goals, personality_values, life_context, doubts_barriers, emotional_landscape, created_at, onboarding_completed'
				)
				.eq('id', sessionId)
				.single();
			...
			console.log('Results fetch result:', {
				results,
				resultsError,
				applicationLetters,
				lettersError
	...
	function getCvStatus() {
		return getSectionStatus(sessionData?.cv_text);
	}
	function getLoveStatus() {
		return getSectionStatus(sessionData?.ikigai_love);
	}
	function getCareAboutStatus() {
		return getSectionStatus(sessionData?.ikigai_care_about);
	}
	function getGoodStatus() {
		return getSectionStatus(sessionData?.ikigai_good_at);
	}
	function getWantToBeStatus() {
		return getSectionStatus(sessionData?.ikigai_want_to_be);
	}
	function getGoalsStatus() {
		return getSectionStatus(sessionData?.goals);
	}
	function getInspiresStatus() {
		return getSectionStatus(sessionData?.ikigai_inspires);
	}
	function getValuesStatus() {
		return getSectionStatus(sessionData?.personality_values);
	}
	function getLifeContextStatus() {
		return getSectionStatus(sessionData?.life_context);
	}
	function getDoubtsStatus() {
		return getSectionStatus(sessionData?.doubts_barriers);
	}
	function getEmotionalStatus() {
		return getSectionStatus(sessionData?.emotional_landscape);
	}
	...
	<div class="mx-auto max-w-6xl">
		...
		<div class="mb-8 pt-8 text-center">
			<div class="mb-4 flex items-center justify-center">
				<h1 class="mr-4 text-4xl font-bold text-gray-800">
					{userData?.user_firstname
						? `${$t('dashboard.welcome_back')}, ${userData.user_firstname}`
						: $t('dashboard.welcome_back')} 🎉
				</h1>
				{#if !isLoading && sessionData}
					<span
						class="inline-flex items-center rounded-full px-3 py-1 text-sm font-medium text-white {getAchievementBadge()
							.color}"
					>
						{getAchievementBadge().icon}
						{getAchievementBadge().text}
					</span>
				{/if}
			</div>
			<p class="mb-2 text-xl text-gray-600">{$t('dashboard.profile_complete')}</p>
			{#if recentActivity}
				<p class="text-sm text-gray-500">
					{$t('dashboard.last_updated')}
					{recentActivity}{userData?.user_email || userData?.email
		...
		{:else if sessionData}
			...
			<div class="mb-8 grid gap-6 md:grid-cols-3">
				<!-- View Results Card -->
				<div
					class="transform rounded-xl bg-white p-6 shadow-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
				>
					<div class="text-center">
						<div
							class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 to-purple-600"
						>
						...
						{#if hasResults && lastUpdated}
							<p class="mb-4 text-sm text-gray-500">
								{$t('dashboard.last_generated')}
								{lastUpdated}
							</p>
						{/if}
						<button
							on:click={viewResults}
							class="w-full transform rounded-lg bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-3 font-semibold text-white transition-all duration-300 hover:scale-105 hover:from-indigo-700 hover:to-purple-700"
							disabled={!hasResults}
						>
				...
				>
					<div class="text-center">
						<div
							class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-green-500 to-emerald-500"
						>
							<svg class="h-8 w-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
								<path
									stroke-linecap="round"
						...
						<h3 class="mb-2 text-xl font-semibold text-gray-800">
							{$t('dashboard.application_letters')}
						</h3>
						<p class="mb-4 text-gray-600">{$t('dashboard.view_and_manage_letters')}</p>
						{#if sessionData.created_at}
							<p class="mb-4 text-sm text-gray-500">
								{$t('dashboard.last_letters_updated')}
								{new Date(sessionData.created_at).toLocaleDateString()}
							</p>
						{/if}
						<button
							on:click={() => goto(`/results/${sessionId}/letters`)}
							class="w-full transform rounded-lg bg-gradient-to-r from-green-500 to-emerald-500 px-6 py-3 font-semibold text-white transition-all duration-300 hover:scale-105 hover:from-green-600 hover:to-emerald-600"
						>
				...
				>
					<div class="text-center">
						...
						<h3 class="mb-2 text-xl font-semibold text-gray-800">
							{$t('dashboard.edit_your_profile')}
						</h3>
						<p class="mb-4 text-gray-600">{$t('dashboard.update_your_responses')}</p>
						{#if sessionData.created_at}
							<p class="mb-4 text-sm text-gray-500">
								{$t('dashboard.last_profile_updated')}
								{new Date(sessionData.created_at).toLocaleDateString()}
							</p>
						{/if}
						<button
							on:click={editAnswers}
							class="w-full transform rounded-lg bg-gradient-to-r from-orange-500 to-red-500 px-6 py-3 font-semibold text-white transition-all duration-300 hover:scale-105 hover:from-orange-600 hover:to-red-600"
						>
			...
			<div class="mb-8 rounded-xl bg-white p-6 shadow-lg">
				...
				<div class="mb-4 h-3 w-full rounded-full bg-gray-200">
					<div
						class="h-3 rounded-full bg-gradient-to-r from-indigo-500 to-purple-500 transition-all duration-1000"
				...
				<div class="grid grid-cols-1 gap-4 md:grid-cols-3">
					...
					<div
						class="cursor-pointer rounded-lg bg-green-50 p-4 text-center transition-colors hover:bg-green-100"
						on:click={viewAllDocuments}
						title={$t('dashboard.view_all_documents')}
						role="button"
						tabindex="0"
						on:keydown={(e) => handleA11yClick(e, viewAllDocuments)}
					>
			...
			{#if !hasResults}
				<div
					class="mb-8 rounded-xl border border-yellow-200 bg-gradient-to-r from-yellow-50 to-orange-50 p-6"
				>
					<div class="flex items-center">
						...
						<div>
							<h3 class="mb-1 text-lg font-semibold text-yellow-800">
								{$t('dashboard.results_in_progress')}
							</h3>
			...
			<div class="rounded-xl bg-white p-8 shadow-lg">
				<div class="mb-6 flex items-center justify-between">
					<h2 class="text-2xl font-semibold text-gray-800">
						{$t('dashboard.your_profile_summary')}
					</h2>
				...
				<div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
					<div
						class="relative rounded-lg bg-gradient-to-r from-blue-50 to-indigo-50 p-4 transition-all duration-300 hover:shadow-md"
					>
						...
						>
							{expandedSections['cv']
								? sessionData.cv_text
								: sessionData.cv_text && sessionData.cv_text.length > 150
									? sessionData.cv_text.substring(0, 150) + '...'
									: sessionData.cv_text}
						</button>
						<button
							class="absolute right-2 bottom-2 rounded bg-indigo-100 px-2 py-1 text-xs text-indigo-700 transition-colors hover:bg-indigo-200"
							on:click={(e) => {
								e.stopPropagation();
								goto(getEditUrl('cv'));
							}}
					...
					<div
						class="relative rounded-lg bg-gradient-to-r from-red-50 to-pink-50 p-4 transition-all duration-300 hover:shadow-md"
					>
						<div class="mb-2 flex items-start justify-between">
							<div class="flex items-center space-x-2">
						...
						>
							{expandedSections['love']
								? sessionData.ikigai_love
								: sessionData.ikigai_love && sessionData.ikigai_love.length > 120
									? sessionData.ikigai_love.substring(0, 120) + '...'
									: sessionData.ikigai_love}
						</button>
						<button
							class="absolute right-2 bottom-2 rounded bg-red-100 px-2 py-1 text-xs text-red-700 transition-colors hover:bg-red-200"
							on:click={(e) => {
								e.stopPropagation();
								goto(getEditUrl('love'));
							}}
						>
							{$t('dashboard.edit')}
						</button>
					...
					<div
						class="relative rounded-lg bg-gradient-to-r from-orange-50 to-amber-50 p-4 transition-all duration-300 hover:shadow-md"
					>
						<div class="mb-2 flex items-start justify-between">
							<div class="flex items-center space-x-2">
								<span class="text-lg">🌍</span>
						...
						>
							{expandedSections['care_about']
								? sessionData.ikigai_care_about
								: sessionData.ikigai_care_about && sessionData.ikigai_care_about.length > 120
									? sessionData.ikigai_care_about.substring(0, 120) + '...'
									: sessionData.ikigai_care_about}
						</button>
						<button
							class="absolute right-2 bottom-2 rounded bg-orange-100 px-2 py-1 text-xs text-orange-700 transition-colors hover:bg-orange-200"
							on:click={(e) => {
								e.stopPropagation();
								goto(getEditUrl('care_about'));
							}}
						>
							{$t('dashboard.edit')}
						</button>
					...
					<div
						class="relative rounded-lg bg-gradient-to-r from-yellow-50 to-orange-50 p-4 transition-all duration-300 hover:shadow-md"
					>
						<div class="mb-2 flex items-start justify-between">
							<div class="flex items-center space-x-2">
								<span class="text-lg">🌟</span>
						...
						>
							{expandedSections['inspires']
								? sessionData.ikigai_inspires
								: sessionData.ikigai_inspires && sessionData.ikigai_inspires.length > 120
									? sessionData.ikigai_inspires.substring(0, 120) + '...'
									: sessionData.ikigai_inspires}
						</button>
						<button
							class="absolute right-2 bottom-2 rounded bg-yellow-100 px-2 py-1 text-xs text-yellow-700 transition-colors hover:bg-yellow-200"
							on:click={(e) => {
								e.stopPropagation();
								goto(getEditUrl('inspires'));
							}}
						>
							{$t('dashboard.edit')}
						</button>
					...
					<div
						class="relative rounded-lg bg-gradient-to-r from-indigo-50 to-purple-50 p-4 transition-all duration-300 hover:shadow-md"
					>
						<div class="mb-2 flex items-start justify-between">
							<div class="flex items-center space-x-2">
								<span class="text-lg">🎭</span>
							...
							<div class="flex items-center space-x-2">
								{#if hasMoreContent(sessionData.ikigai_want_to_be, 120)}
									<button
										type="button"
										class="text-sm font-medium text-indigo-800 hover:text-indigo-800"
										on:click={(e) => {
											e.stopPropagation();
											toggleSection('want_to_be');
										}}
									>
						...
						>
							{expandedSections['want_to_be']
								? sessionData.ikigai_want_to_be
								: sessionData.ikigai_want_to_be && sessionData.ikigai_want_to_be.length > 120
									? sessionData.ikigai_want_to_be.substring(0, 120) + '...'
									: sessionData.ikigai_want_to_be}
						</button>
						<button
							class="absolute right-2 bottom-2 rounded bg-indigo-100 px-2 py-1 text-xs text-indigo-700 transition-colors hover:bg-indigo-200"
					...
					<div
						class="relative rounded-lg bg-gradient-to-r from-green-50 to-emerald-50 p-4 transition-all duration-300 hover:shadow-md"
					>
						<div class="mb-2 flex items-start justify-between">
							<div class="flex items-center space-x-2">
								<span class="text-lg">⭐</span>
						...
						>
							{expandedSections['good']
								? sessionData.ikigai_good_at
								: sessionData.ikigai_good_at && sessionData.ikigai_good_at.length > 120
									? sessionData.ikigai_good_at.substring(0, 120) + '...'
									: sessionData.ikigai_good_at}
						</button>
						<button
							class="absolute right-2 bottom-2 rounded bg-green-100 px-2 py-1 text-xs text-green-700 transition-colors hover:bg-green-200"
							on:click={(e) => {
								e.stopPropagation();
								goto(getEditUrl('good_at'));
							}}
						>
							{$t('dashboard.edit')}
						</button>
					...
					<div
						class="relative rounded-lg bg-gradient-to-r from-purple-50 to-violet-50 p-4 transition-all duration-300 hover:shadow-md"
					>
						<div class="mb-2 flex items-start justify-between">
							<div class="flex items-center space-x-2">
								<span class="text-lg">🎯</span>
						...
						>
							{expandedSections['goals']
								? sessionData.goals
								: sessionData.goals && sessionData.goals.length > 120
									? sessionData.goals.substring(0, 120) + '...'
									: sessionData.goals}
						</button>
						<button
							class="absolute right-2 bottom-2 rounded bg-purple-100 px-2 py-1 text-xs text-purple-700 transition-colors hover:bg-purple-200"
							on:click={(e) => {
								e.stopPropagation();
								goto(getEditUrl('goals'));
							}}
						>
							{$t('dashboard.edit')}
						</button>
					...
					<div
						class="relative rounded-lg bg-gradient-to-r from-cyan-50 to-blue-50 p-4 transition-all duration-300 hover:shadow-md"
					>
						<div class="mb-2 flex items-start justify-between">
							<div class="flex items-center space-x-2">
								<span class="text-lg">💎</span>
						...
						>
							{expandedSections['values']
								? sessionData.personality_values
								: sessionData.personality_values && sessionData.personality_values.length > 120
									? sessionData.personality_values.substring(0, 120) + '...'
									: sessionData.personality_values}
						</button>
						<button
							class="absolute right-2 bottom-2 rounded bg-cyan-100 px-2 py-1 text-xs text-cyan-700 transition-colors hover:bg-cyan-200"
							on:click={(e) => {
								e.stopPropagation();
								goto(getEditUrl('personality_values'));
							}}
						>
							{$t('dashboard.edit')}
						</button>
					...
					<div
						class="relative rounded-lg bg-gradient-to-r from-emerald-50 to-teal-50 p-4 transition-all duration-300 hover:shadow-md"
					>
						<div class="mb-2 flex items-start justify-between">
							<div class="flex items-center space-x-2">
								<span class="text-lg">🏠</span>
						...
						>
							{expandedSections['life_context']
								? sessionData.life_context
								: sessionData.life_context && sessionData.life_context.length > 120
									? sessionData.life_context.substring(0, 120) + '...'
									: sessionData.life_context}
						</button>
						<button
							class="absolute right-2 bottom-2 rounded bg-emerald-100 px-2 py-1 text-xs text-emerald-700 transition-colors hover:bg-emerald-200"
							on:click={(e) => {
								e.stopPropagation();
								goto(getEditUrl('life_context'));
							}}
						>
							{$t('dashboard.edit')}
						</button>
					...
					<div
						class="relative rounded-lg bg-gradient-to-r from-amber-50 to-orange-50 p-4 transition-all duration-300 hover:shadow-md"
					>
						<div class="mb-2 flex items-start justify-between">
							<div class="flex items-center space-x-2">
								<span class="text-lg">🤔</span>
						...
						>
							{expandedSections['doubts']
								? sessionData.doubts_barriers
								: sessionData.doubts_barriers && sessionData.doubts_barriers.length > 120
									? sessionData.doubts_barriers.substring(0, 120) + '...'
									: sessionData.doubts_barriers}
						</button>
						<button
							class="absolute right-2 bottom-2 rounded bg-amber-100 px-2 py-1 text-xs text-amber-700 transition-colors hover:bg-amber-200"
							on:click={(e) => {
								e.stopPropagation();
								goto(getEditUrl('doubts_barriers'));
							}}
						>
							{$t('dashboard.edit')}
						</button>
					...
					<div
						class="relative rounded-lg bg-gradient-to-r from-rose-50 to-pink-50 p-4 transition-all duration-300 hover:shadow-md"
					>
						<div class="mb-2 flex items-start justify-between">
							<div class="flex items-center space-x-2">
								<span class="text-lg">💭</span>
						...
						>
							{expandedSections['emotional']
								? sessionData.emotional_landscape
								: sessionData.emotional_landscape && sessionData.emotional_landscape.length > 120
									? sessionData.emotional_landscape.substring(0, 120) + '...'
									: sessionData.emotional_landscape}
						</button>
						<button
							class="absolute right-2 bottom-2 rounded bg-rose-100 px-2 py-1 text-xs text-rose-700 transition-colors hover:bg-rose-200"
							on:click={(e) => {
								e.stopPropagation();
								goto(getEditUrl('emotional_landscape'));
							}}
						>
							{$t('dashboard.edit')}
						</button>
		...
		{:else}
			<div class="rounded-xl bg-white p-8 text-center shadow-lg">
				...
				<h3 class="mb-2 text-xl font-semibold text-gray-800">
					{$t('dashboard.could_not_load_session_details')}
				</h3>
				<p class="mb-4 text-gray-600">{$t('dashboard.issue_loading_profile_information')}</p>
				<div class="space-y-2">
					<button
						on:click={() => window.location.reload()}
						class="mr-2 rounded-lg bg-indigo-600 px-6 py-2 text-white hover:bg-indigo-700"
					>
