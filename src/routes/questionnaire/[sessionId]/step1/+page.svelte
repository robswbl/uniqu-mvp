<script lang="ts">
	...
	onMount(async () => {
		const { data, error } = await supabase
			...
			.select(
				'goals, personality_values, life_context, doubts_barriers, emotional_landscape, core_summary'
			)
			.eq('id', sessionId)
			.single();
	...
	<div class="mx-auto max-w-4xl">
			...
			<div class="flex items-center space-x-4">
				{#if hasUnsavedChanges}
					...
					>
						...
						<span class="text-sm text-yellow-800">{$t('step1.overview.unsaved_changes')}</span>
					</div>
				{/if}
				{#if saveStatus}
					<div
		...
		<div class="mb-8 text-center">
			<div class="mb-4 flex items-center justify-center space-x-2">
				...
				<div
					class="flex h-8 w-8 items-center justify-center rounded-full bg-gray-200 text-sm font-semibold text-gray-400"
				>
					2
				</div>
				<div class="h-0.5 w-16 bg-gray-300"></div>
			...
			<h1 class="mb-2 text-3xl font-bold text-gray-800">{$t('step1.overview.heading')}</h1>
		...
		<form method="POST" use:enhance={handleSubmit} class="space-y-8">
			...
			<div class="rounded-2xl bg-white p-8 shadow-xl">
				<div class="mb-6 flex items-center space-x-3">
					...
					<h2 class="text-2xl font-semibold text-gray-800">
						{$t('step1.overview.life_context_title')}
					</h2>
				</div>
				<div class="space-y-4">
					<div class="rounded-lg bg-green-50 p-4">
						<p class="mb-2 text-sm text-green-800">
							💡 <strong>{$t('step1.overview.life_context_explainer')}</strong>
						</p>
					...
					<label for="life-context-textarea" class="sr-only"
						>{$t('step1.overview.life_context_title')}</label
					>
					<textarea
			...
			<div class="rounded-2xl bg-white p-8 shadow-xl">
				<div class="mb-6 flex items-center space-x-3">
					...
					<h2 class="text-2xl font-semibold text-gray-800">
						{$t('step1.overview.personality_title')}
					</h2>
				</div>
				<div class="space-y-4">
					<div class="rounded-lg bg-blue-50 p-4">
						<p class="mb-2 text-sm text-blue-800">
							💡 <strong>{$t('step1.overview.personality_explainer')}</strong>
						</p>
					...
					<label for="personality-textarea" class="sr-only"
						>{$t('step1.overview.personality_title')}</label
					>
					<textarea
				...
				<div class="space-y-4">
					<div class="rounded-lg bg-purple-50 p-4">
						<p class="mb-2 text-sm text-purple-800">
							💡 <strong>{$t('step1.overview.goals_explainer')}</strong>
						</p>
				...
				<div class="space-y-4">
					<div class="rounded-lg bg-indigo-50 p-4">
						<p class="mb-2 text-sm text-indigo-800">
							💡 <strong>{$t('step1.overview.doubts_explainer')}</strong>
						</p>
			...
			<div class="rounded-2xl bg-white p-8 shadow-xl">
				<div class="mb-6 flex items-center space-x-3">
					...
					<h2 class="text-2xl font-semibold text-gray-800">
						{$t('step1.overview.emotional_title')}
					</h2>
				</div>
				<div class="space-y-4">
					<div class="rounded-lg bg-indigo-50 p-4">
						<p class="mb-2 text-sm text-indigo-800">
							💡 <strong>{$t('step1.overview.emotional_explainer')}</strong>
						</p>
						<ul class="space-y-1 text-sm text-indigo-700">
					...
					<label for="emotional-textarea" class="sr-only"
						>{$t('step1.overview.emotional_title')}</label
					>
					<textarea
			...
			<div class="flex items-center justify-between pt-6">
				<button
					type="button"
					on:click={goToDashboard}
					class="flex items-center space-x-2 rounded-lg px-6 py-3 text-gray-600 transition-colors hover:bg-gray-50 hover:text-gray-800"
				...
				<button
					...
					class="flex items-center space-x-3 rounded-lg bg-gradient-to-r from-indigo-600 to-purple-500 px-8 py-4 font-semibold text-white shadow-lg transition-colors hover:from-indigo-700 hover:to-purple-600"
					style="display: flex !important; visibility: visible !important; opacity: 1 !important;"
				>
