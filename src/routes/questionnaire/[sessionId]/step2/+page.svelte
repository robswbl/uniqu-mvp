<script lang="ts">
	...
	onMount(async () => {
		...
		if (sessionData?.cv_text) {
			...
			saveStatus = $t('step2.loaded_existing_data');
			setTimeout(() => {
	...
	async function saveProgress() {
		const { error } = await supabase
			...
			.eq('id', sessionId);
		if (error) {
			saveStatus = $t('step2.error_saving');
		} else {
	...
	async function handleFileUpload(event: Event) {
		...
		uploadStatus = $t('step2.uploading');
		uploadProgress = 0;
		...
		await new Promise<void>((resolve, reject) => {
			...
			xhr.onload = () => {
				if (xhr.status >= 200 && xhr.status < 300) {
					uploadStatus = $t('step2.upload_complete');
					uploadProgress = 100;
					resolve();
				} else {
					uploadStatus = $t('step2.upload_failed');
					uploadProgress = 0;
					reject();
				}
			};
		...
		pollingCvText = true;
		uploadStatus = $t('step2.processing_cv');
		pollForCvText();
	}
	...
	function pollForCvText() {
		...
		pollingInterval = setInterval(async () => {
			...
			if (data?.cv_text && data.cv_text.length > 0) {
				...
				pollingCvText = false;
				uploadStatus = $t('step2.cv_processed');
				if (pollingInterval) clearInterval(pollingInterval);
			}
		}, 2000);
	}
...
<svelte:head>
	<title>{$t('step2.title')} - {$t('app.title')}</title>
</svelte:head>
