...
<script>
	...
	async function fetchData() {
		try {
			...
			if (!contentCheckError && allLetters) {
				const lettersWithContent = allLetters.filter(
					(letter) => letter.content_html || letter.letter_content_html
				);
	...
	async function analyzePainPoints() {
		...
		try {
			...
			if (!webhookResponse.ok) {
				...
				throw new Error(
					`Pain points analysis failed: ${webhookResponse.status} ${webhookResponse.statusText}${errorDetails ? ` - ${errorDetails}` : ''}`
				);
			}
	...
	function pollForPainPointsAnalysis(letterId) {
		...
		error = {
			...
			details:
				'Analyzing company websites and identifying pain points. This process can take 5-10 minutes depending on the number of companies being analyzed.\n\n• Please wait while the analysis continues\n• You can leave this page and check back later\n• Results will appear automatically when complete',
			timestamp: new Date().toISOString(),
			originalError: null
		};
				...
				try {
					...
					if (!dbError && letterData && letterData.status === 'error') {
						...
						error = {
							...
							details:
								'The server-side analysis process encountered an error. This could be due to:\n\n• The company website being temporarily unavailable\n• The scraping service being overloaded\n• A configuration issue with the workflow\n\nPlease try again with a different company URL.',
							timestamp: new Date().toISOString(),
					...
					error = {
						...
						details:
							'Failed to check the status of the pain points analysis. This could be due to:\n\n• Network connectivity issues\n• Database connection problems\n• Server-side errors\n\nPlease try again.',
						timestamp: new Date().toISOString(),
	...
	async function generateLetter() {
		...
		try {
			...
			try {
				...
				alert(
					`Letter record created, but AI generation failed.\n\nError: ${webhookError.message}\n\nYou can try regenerating later.`
				);
			}
		} catch (err) {
		...
		function poll() {
			setTimeout(async () => {
				...
				if (
					!letterError &&
					letterData &&
					letterData.company_name !== 'Job Analysis in Progress...'
				) {
					...
					applicationLetters = applicationLetters.map((letter) =>
						letter.id === letterId
							? {
									...letter,
									company_name: letterData.company_name,
				...
				if (
					!contentError &&
					letterContent &&
					(letterContent.content_html || letterContent.letter_content_html)
				) {
					...
					applicationLetters = applicationLetters.map((letter) =>
						letter.id === letterId
							? {
									...letter,
									content_html: letterContent.content_html || letterContent.letter_content_html,
									letter_content_html: letterContent.letter_content_html,
									status: letterContent.status
								}
							: letter
					);
				...
				if (!contentError && letterContent && letterContent.status === 'completed') {
					...
					applicationLetters = applicationLetters.map((letter) =>
						letter.id === letterId ? { ...letter, status: letterContent.status } : letter
					);
				...
				console.log(
					'Checking application_letters for letter:',
					letterId,
					'Data:',
					letterContentData,
					'Error:',
				...
				if (
					!letterContentError &&
					letterContentData &&
					(letterContentData.letter_content_html || letterContentData.content_html)
				) {
					console.log(
						'Content found in application_letters for letter:',
						letterId,
						letterContentData
					);
					generatedLetterIds.add(letterId.toString());
					...
					applicationLetters = applicationLetters.map((letter) =>
						letter.id === letterId
							? {
									...letter,
									letter_content_html: letterContentData.letter_content_html,
									content_html: letterContentData.content_html
								}
							: letter
					);
	...
	async function updateNotes(letterId, notes) {
		try {
			...
			// Update local state
			applicationLetters = applicationLetters.map((letter) =>
				letter.id === letterId ? { ...letter, notes: notes.trim() || null } : letter
			);
	...
	async function triggerWebhookForLetter(letter) {
		try {
			...
			alert(
				`✅ Webhook triggered successfully for ${letter.company_name}!\n\nCheck n8n workflow and generated documents.`
			);
		} catch (error) {
		...
		try {
			...
			if (error) {
				...
				if (error.code === 'PGRST116') {
					alert(
						'Letter content not yet generated. Please wait for the AI to finish processing, then try again.'
					);
				} else {
					throw error;
				}
	...
	async function updatePlaceholderLetter(letter) {
		if (
			!confirm(
				`Update the placeholder title for this letter? This will allow you to manually set the company name.`
			)
		) {
			return;
		}
		...
		const newCompanyName = prompt(
			'Enter the company name for this letter:',
			letter.company_name === 'Job Analysis in Progress...' ? '' : letter.company_name
		);
		...
		try {
			...
			applicationLetters = applicationLetters.map((l) =>
				l.id === letter.id ? { ...l, company_name: newCompanyName.trim() } : l
			);
	...
	async function cleanupDuplicateOriginalVersions(letterId) {
		try {
			...
			if (originalVersions && originalVersions.length > 1) {
				console.log(
					`Found ${originalVersions.length} original versions for letter ${letterId}, cleaning up duplicates`
				);
				...
				if (deleteError) {
					...
					console.log(
						`Successfully deleted ${versionsToDelete.length} duplicate original versions`
					);
				}
			}
		} catch (err) {
	...
	async function fetchLetterVersions(letterId) {
		try {
			...
			console.log(
				`[fetchLetterVersions] Raw rewrite versions from DB for letter ${letterId}:`,
				rewriteVersions
			);
			...
			if (rewriteVersions && rewriteVersions.length > 0) {
				console.log(
					`[fetchLetterVersions] Found ${rewriteVersions.length} versions for letter ${letterId}:`,
					rewriteVersions
				);
				...
				if (existingOriginal) {
					...
					if (letterError) {
						console.error(
							`[fetchLetterVersions] Error fetching original letter ${letterId}:`,
							letterError
						);
						return;
					}
					...
					console.log(
						`[fetchLetterVersions] Found ${allVersions.length} total versions for letter ${letterId}:`,
						allVersions
					);
	...
	async function regenerateLetter(letterId) {
		try {
			...
			if (!existingLetter) {
				...
				throw new Error(
					'Letter not found in current session. Please refresh the page and try again.'
				);
			}
			...
			console.log(
				...
				applicationLetters.map((l) => ({ id: l.id, company: l.company_name }))
			);
			...
			console.log('Letter found:', {
				id: letterId,
				hasContent: !!(letter.content_html || letter.letter_content_html)
			});
			...
			if (!originalContent) {
				...
				throw new Error(
					'This letter has no content to regenerate. Please wait for the original letter to finish generating first.'
				);
			}
			...
			if (err.message.includes('not found')) {
				...
				userMessage +=
					'Please wait for the original letter to finish generating before creating variations.';
		...
		function poll() {
			setTimeout(async () => {
				try {
					...
					console.log(
						`[Polling] Query: SELECT * FROM application_letter_versions WHERE application_letter_id = ${letterId}`
					);
	...
	function parseMarkdown(text) {
		...
		text = text.replace(
			/^##\s+(.*?)$/gm,
			'<h2 class="text-xl font-semibold text-gray-800 mt-6 mb-3">$1</h2>'
		);
		...
		subscription = supabase
			...
			.on(
				...
				async (payload) => {
					...
					if (
						payload.new.content_html ||
						payload.new.letter_content_html ||
			...
			.on(
				...
				async (payload) => {
					...
					if (
						payload.new.document_type === 'application_letter' &&
						payload.new.application_letter_id
					) {
			...
			console.log('Form state:', {
				...
				disabled:
					...
					(newLetterType === 'spontaneous' &&
						((!selectedCompany && !customCompany.trim()) || !painPoints.trim() || !address.trim()))
			});
		}
	}
...
<div class="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-100 p-4">
	<div class="mx-auto max-w-6xl">
		...
		<div class="mb-8">
			...
			<div class="flex items-center justify-between">
				<div>
					<h1 class="mb-2 text-3xl font-bold text-gray-900">{$t('letters.heading')}</h1>
					<p class="text-gray-600">{$t('letters.subheading')}</p>
				</div>
				<div class="relative" bind:this={dropdownRef}>
					<button
						on:click={() => (showNewLetterDropdown = !showNewLetterDropdown)}
						class="flex items-center space-x-2 rounded-lg bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-3 font-medium text-white shadow-lg transition-all duration-200 hover:from-indigo-700 hover:to-purple-700"
						type="button"
					...
					{#if showNewLetterDropdown}
						<div
							class="absolute right-0 z-50 mt-2 w-64 rounded-lg border border-gray-200 bg-white py-2 shadow-lg"
						>
							<button
								class="block w-full px-4 py-3 text-left text-gray-700 transition-colors hover:bg-gray-100"
								type="button"
		...
		{:else if error}
			{#if typeof error === 'object' && error.type === 'processing'}
				...
				<div class="rounded-xl border border-blue-200 bg-blue-50 p-6 text-center">
					...
					<p class="mb-3 font-medium text-blue-600">
						{error.details}
					</p>
					<!-- Progress Bar -->
					<div class="mb-4 h-2 w-full rounded-full bg-blue-200">
						<div
			...
			{:else}
				...
				<div class="rounded-xl border border-red-200 bg-red-50 p-6 text-center">
					...
					<p class="mb-3 font-medium text-red-600">
						{typeof error === 'string' ? error : error.message}
					</p>
					{#if typeof error === 'object' && error.details}
						<div class="mb-4 rounded-lg bg-red-100 p-3 text-left text-sm text-red-600">
			...
			{#if showNewLetterForm}
				<div class="mb-8 rounded-xl border border-gray-200 bg-white p-6 shadow-lg">
					...
					<div class="space-y-4">
						<!-- Language Selector -->
						<div>
							<label for="letter-language" class="mb-2 block text-sm font-medium text-gray-700">
								{$t('letters.letter_language_label')}
							</label>
							<select
								id="letter-language"
								bind:value={newLetterLanguage}
								class="w-full rounded-lg border border-gray-300 p-3 focus:border-transparent focus:ring-2 focus:ring-indigo-500"
							>
						...
						<!-- Tone Selector -->
						<div>
							<label for="letter-tone" class="mb-2 block text-sm font-medium text-gray-700">
						...
						{#if newLetterType === 'job'}
							<div>
								<label for="job-url" class="mb-2 block text-sm font-medium text-gray-700">
									<span class="text-red-500">*</span>
									{$t('letters.job_url_label')}
								</label>
								<input
									id="job-url"
									type="url"
									...
									required
								/>
								<p class="mt-1 text-xs text-gray-500">{$t('letters.job_url_hint')}</p>
								{#if linkedinUrlWarning}
									<div class="mt-2 rounded-lg border border-red-200 bg-red-50 p-3">
										<div class="flex items-start">
											<svg
												class="mt-0.5 mr-2 h-5 w-5 flex-shrink-0 text-red-400"
												fill="none"
												stroke="currentColor"
												viewBox="0 0 24 24"
											>
						...
						{#if newLetterType === 'spontaneous'}
							...
							<div class="mb-2 text-xs text-gray-500">
								Debug: Step {spontaneousStep}, Pain Points Complete: {painPointsAnalysisComplete},
								Address: {address ? 'Set' : 'Not set'}
							</div>
							<!-- Step 1: Company and URL -->
							{#if spontaneousStep === 1}
								<!-- Company Name Field -->
								<div>
									<label for="custom-company" class="mb-2 block text-sm font-medium text-gray-700">
								...
								<div>
									<label
										for="company-portal-url"
										class="mb-2 block text-sm font-medium text-gray-700"
									>
										<span class="text-red-500">*</span>
										{$t('letters.company_portal_url_label')}
									</label>
									<input
										...
										required
									/>
									<p class="mt-1 text-xs text-gray-500">{$t('letters.company_portal_url_hint')}</p>
									{#if jobsChWarning}
										<div class="mt-2 rounded-lg border border-red-200 bg-red-50 p-3">
											<div class="flex items-start">
												<svg
							...
							{:else}
								...
								<div>
									<label for="pain-points" class="mb-2 block text-sm font-medium text-gray-700">
										<span class="text-green-500">✓</span>
										{$t('letters.key_pain_points')}
									</label>
									...
									<p class="mt-1 text-xs text-green-600">{$t('letters.pain_points_analyzed')}</p>
								</div>
								...
								<div>
									...
									<textarea
										...
										required
									></textarea>
									<p class="mt-1 text-xs text-gray-500">{$t('letters.address_hint')}</p>
								</div>
							{/if}
						{/if}
						...
						<div class="flex items-center space-x-4 pt-4">
							{#if newLetterType === 'spontaneous' && spontaneousStep === 1}
								...
								<button
									...
									disabled={analyzingPainPoints ||
										!customCompany.trim() ||
										!companyPortalUrl.trim() ||
										jobsChWarning}
									class="flex items-center space-x-2 rounded-lg bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-3 font-medium text-white transition-all duration-200 hover:from-blue-700 hover:to-indigo-700 disabled:cursor-not-allowed disabled:opacity-50"
									type="button"
							...
							{:else}
								...
								<button
									...
									disabled={generating ||
										...
										(newLetterType === 'spontaneous' &&
											spontaneousStep === 2 &&
											(!painPoints.trim() || !address.trim()))}
									class="flex items-center space-x-2 rounded-lg bg-gradient-to-r from-green-600 to-emerald-600 px-6 py-3 font-medium text-white transition-all duration-200 hover:from-green-700 hover:to-emerald-700 disabled:cursor-not-allowed disabled:opacity-50"
									type="button"
							...
							<button
								...
								class="rounded-lg px-4 py-3 text-gray-600 transition-colors hover:bg-gray-50 hover:text-gray-800"
								type="button"
								aria-label="Cancel"
							>
								{$t('letters.cancel')}
					...
					<div class="py-12 text-center">
						<div class="mx-auto max-w-md rounded-xl bg-white p-8 shadow-lg">
							...
							<h3 class="mb-2 text-xl font-semibold text-gray-900">
								{$t('letters.no_letters_yet')}
							</h3>
							<p class="mb-6 text-gray-600">{$t('letters.no_letters_desc')}</p>
							<button
								on:click={() => (showNewLetterForm = true)}
								class="rounded-lg bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-3 font-medium text-white transition-all duration-200 hover:from-indigo-700 hover:to-purple-700"
								type="button"
				...
				{:else}
					...
					<div class="mb-4 flex items-center justify-end">
						<div class="text-sm text-gray-500">
							{applicationLetters.length} application letter{applicationLetters.length !== 1
								? 's'
								: ''}
						</div>
					</div>
							...
							>
									...
									<div class="flex items-center space-x-4">
										...
										{#if letter.status === 'draft' && !isLetterGenerated(letter)}
											{#if letter.pain_points && !letter.content_html && !letter.letter_content_html}
												...
												<div class="flex items-center space-x-2">
													...
													<button
														...
														class="flex items-center space-x-1 rounded-lg p-2 text-red-600 transition-colors hover:bg-red-50 hover:text-red-800"
														type="button"
											...
											{:else if pollingErrors[typeof letter.id === 'string' ? parseInt(letter.id, 10) : letter.id]}
												<div class="flex min-w-[180px] flex-col items-end">
													<div class="mb-1 flex items-center space-x-2">
														...
														<span class="font-medium text-red-500"
															>{$t('letters.generation_failed')}</span
														>
													</div>
													<div class="h-2 w-40 rounded-full bg-gray-200">
														<div
															class="h-2 rounded-full bg-gradient-to-r from-red-300 to-red-400"
															style="width: 100%"
														></div>
													</div>
												...
												<div class="flex min-w-[180px] flex-col items-end">
													<div class="mb-1 flex items-center space-x-2">
														...
														<span class="font-medium text-indigo-700"
															>{$t('letters.generating')}</span
														>
													</div>
													<div class="h-2 w-40 rounded-full bg-gray-200">
														<div
															class="h-2 rounded-full bg-gradient-to-r from-indigo-500 to-purple-500 transition-all duration-200"
															style="width: {Math.round(
																(letterProgress[
																	typeof letter.id === 'string'
																		? parseInt(letter.id, 10)
																		: letter.id
																] || 0) * 100
															)}%"
														></div>
												...
												<div class="flex min-w-[180px] flex-col items-end">
													<div class="mb-1 flex items-center space-x-2">
														...
														<span class="font-medium text-gray-500"
															>{$t('letters.still_generating')}</span
														>
													</div>
													<div class="h-2 w-40 rounded-full bg-gray-200">
														<div
															class="h-2 rounded-full bg-gradient-to-r from-gray-300 to-gray-400"
															style="width: 100%"
														></div>
													</div>
													<div class="mt-1 text-xs text-gray-400">
														{$t('letters.taking_longer')}
													</div>
												</div>
											{/if}
										{:else}
											<div class="flex items-center space-x-2">
												...
												<button
													...
													class="flex items-center space-x-2 rounded-lg border border-green-700 bg-green-600 px-4 py-2 text-base font-semibold text-white shadow-md transition-colors hover:bg-green-700"
													type="button"
												...
												{#if letter.content_html || letter.letter_content_html}
													<button
														...
														class="flex items-center space-x-1 rounded-lg p-2 text-indigo-600 transition-colors hover:bg-indigo-50 hover:text-indigo-800"
														type="button"
												...
												{#if letter.company_name === 'Job Analysis in Progress...' && letter.job_url}
													<button
														...
														class="flex items-center space-x-1 rounded-lg p-2 text-blue-600 transition-colors hover:bg-blue-50 hover:text-blue-800"
														type="button"
								...
								<div class="mb-4 space-y-2">
											...
											{#if letter.status === 'draft' && !letter.content_html && !letter.letter_content_html}
												<div class="mt-3 flex justify-center">
													<button
														...
														class="flex items-center space-x-2 rounded-lg border border-blue-700 bg-blue-600 px-6 py-3 text-base font-semibold text-white shadow-md transition-colors hover:bg-blue-700"
														type="button"
													...
													>
														...
														<span
															>{letter.address && letter.address.trim()
																? $t('letters.generate_letter_now')
																: $t('letters.continue_letter_generation')}</span
														>
													</button>
												</div>
											{/if}
									...
									{#if continuingLetterId === letter.id}
										<div class="mt-4 rounded-lg border border-blue-200 bg-blue-50 p-4">
											<div class="mb-3">
												<label
													for="inline-address-{letter.id}"
													class="mb-2 block text-sm font-medium text-gray-700"
												>
													<span class="text-red-500">*</span>
													{$t('letters.company_recipient')}
												</label>
												<textarea
											...
											<div class="flex items-center space-x-3">
												<button
													...
													class="flex items-center space-x-2 rounded-lg bg-green-600 px-4 py-2 font-medium text-white transition-colors hover:bg-green-700 disabled:cursor-not-allowed disabled:opacity-50"
													type="button"
													aria-label={$t('letters.generate_letter')}
												>
													{#if generating}
												...
												<button
													...
													class="rounded-lg px-3 py-2 text-gray-600 transition-colors hover:bg-gray-50 hover:text-gray-800"
													type="button"
									...
									{:else if letter.address}
										<div>
											...
											<p
												class="mt-1 rounded bg-gray-50 p-2 text-sm whitespace-pre-line text-gray-700"
											>
												{letter.address}
											</p>
										</div>
								...
								<div class="mb-4 rounded-lg border border-gray-200 bg-gray-50 p-4">
									...
									{#if letter.job_url}
										<div class="mb-3 text-sm text-gray-600">
											...
											<a
												...
												title={letter.job_url}
											>
												{letter.job_url.length > 50
													? letter.job_url.substring(0, 47) + '...'
													: letter.job_url}
											</a>
										</div>
									{/if}
									...
									<div class="mb-3 flex items-center space-x-4 text-sm text-gray-500">
										...
										{#if letterVersions[letter.id] && letterVersions[letter.id].length > 1}
											<button
												on:click={() => viewLetter(letter.id)}
												class="flex cursor-pointer items-center rounded px-2 py-1 text-indigo-600 transition-colors hover:bg-indigo-50 hover:text-indigo-800"
												type="button"
											...
											>
												...
												{letterVersions[letter.id].length}
												{$t('letters.versions')}
											</button>
										{/if}
									</div>
									...
									<div class="flex items-center space-x-8">
										...
										<div class="flex flex-col">
											<span class="mb-1 pl-4 text-xs text-gray-500"
												>{$t('letters.set_application_status')}</span
											>
											<select
												value={letter.status}
												on:change={(e) => handleStatusChange(letter.id, e)}
												class="min-w-[200px] rounded-full border-0 px-4 py-2 text-xs font-medium focus:ring-2 focus:ring-indigo-500 {getStatusColor(
													letter.status
												)}"
											...
											>
												{#each statusOptions as option}
													<option value={option.value}
														>{$t(`letters.status.${option.value}`)}</option
													>
												{/each}
											</select>
										</div>
												...
												<div class="flex flex-wrap items-center gap-2">
													{#each getStatusTimeline(letter) as event}
														{#if editingStatusDates[letter.id]?.[event.field] !== undefined}
															...
															<div class="flex items-center space-x-2">
																<input
																	...
																	class="rounded border border-gray-300 px-2 py-1 text-xs focus:ring-1 focus:ring-indigo-500"
																/>
																<button
														...
														{:else}
															...
															<button
																on:click={() =>
																	startEditingStatusDate(letter.id, event.field, event.date)}
																class="rounded-full bg-gray-100 px-2 py-1 text-xs {event.color} cursor-pointer font-medium transition-colors hover:bg-gray-200"
																type="button"
								...
								<div class="mb-6 border-t border-gray-100 pt-4">
									<div class="rounded-lg border border-gray-200 bg-gray-50 p-4">
										<div class="mb-2 flex items-start justify-between">
											<label for="notes-{letter.id}" class="text-sm font-medium text-gray-700"
												>{$t('letters.notes')}</label
											>
											...
											{:else}
												<button
													...
													class="text-xs text-indigo-600 transition-colors hover:text-indigo-800"
													type="button"
													aria-label={letter.notes
														? $t('letters.edit_notes')
														: $t('letters.add_note')}
												>
										...
										{#if editingNotes[letter.id] !== undefined}
											<textarea
												...
												class="w-full resize-none rounded-lg border border-gray-300 p-3 text-sm focus:border-transparent focus:ring-2 focus:ring-indigo-500"
												rows="3"
												aria-label="Letter notes"
											></textarea>
										{:else}
											<p class="min-h-[1.5rem] text-sm text-gray-600 italic">
								...
								{#if letter.cv_tagline || letter.cv_keywords || letter.cv_managementsummary || letter.cv_tips}
									<div class="border-t border-gray-100 pt-4">
										<h4 class="mb-6 flex items-center text-xl font-bold text-gray-900">
											<svg
										...
										<div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
											...
											{#if letter.cv_tagline}
												<div class="space-y-2">
													<div
														class="relative min-h-[100px] rounded-xl border border-blue-200 bg-gradient-to-br from-blue-50 to-blue-100 p-4 shadow-sm transition-shadow duration-200 hover:shadow-md"
													>
														<div class="mb-3 flex items-center justify-between">
															<h5 class="flex items-center text-sm font-semibold text-blue-800">
																<svg
																	class="mr-2 h-4 w-4"
																	fill="none"
																	stroke="currentColor"
															...
															<button
																...
																class="rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-medium text-white shadow-sm transition-all duration-200 hover:bg-blue-700"
																type="button"
											...
											{#if letter.cv_keywords}
												<div class="space-y-2">
													<div
														class="relative min-h-[100px] rounded-xl border border-green-200 bg-gradient-to-br from-green-50 to-green-100 p-4 shadow-sm transition-shadow duration-200 hover:shadow-md"
													>
														<div class="mb-3 flex items-center justify-between">
															<h5 class="flex items-center text-sm font-semibold text-green-800">
																<svg
																	class="mr-2 h-4 w-4"
																	fill="none"
																	stroke="currentColor"
																...
																>
																	<path
																		...
																		stroke-linejoin="round"
																		stroke-width="2"
																		d="M7 7.732A4 4 0 0115.732 4h1.086a1 1 0 01.707.293l3.414 3.414a1 1 0 01.293.707V15.732A4 4 0 0112.268 20H7.268A4 4 0 013 15.732V8.268a1 1 0 01.293-.707L6.707 4.293A1 1 0 017.414 4H8.268A4 4 0 0112 7.732z"
																	/>
																</svg>
															...
															<button
																...
																class="rounded-lg bg-green-600 px-3 py-1.5 text-xs font-medium text-white shadow-sm transition-all duration-200 hover:bg-green-700"
																type="button"
																title="Copy to clipboard"
															>
																{$t('letters.copy')}
														...
														<div class="text-sm text-gray-700">
															...
															<div class="flex flex-wrap gap-2">
																{#each formatCvKeywords(letter.cv_keywords) as keyword}
																	<span
																		class="inline-block rounded-full border border-green-200 bg-green-100 px-2 py-1 text-xs text-green-800 transition-colors hover:bg-green-200"
																	>
																		{keyword}
																	</span>
																{/each}
											...
											{#if letter.cv_managementsummary}
												<div class="space-y-2 lg:col-span-2">
													<div
														class="relative min-h-[140px] rounded-xl border border-purple-200 bg-gradient-to-br from-purple-50 to-purple-100 p-4 shadow-sm transition-shadow duration-200 hover:shadow-md"
													>
														<div class="mb-3 flex items-center justify-between">
															<h5 class="flex items-center text-sm font-semibold text-purple-800">
																<svg
																	class="mr-2 h-4 w-4"
																	fill="none"
																	stroke="currentColor"
															...
															<button
																...
																class="rounded-lg bg-purple-600 px-3 py-1.5 text-xs font-medium text-white shadow-sm transition-all duration-200 hover:bg-purple-700"
																type="button"
																title="Copy to clipboard"
															>
																{$t('letters.copy')}
											...
											{#if letter.cv_tips}
												<div class="space-y-2 lg:col-span-2">
													<div
														class="relative min-h-[160px] rounded-xl border border-orange-200 bg-gradient-to-br from-orange-50 to-orange-100 p-4 shadow-sm transition-shadow duration-200 hover:shadow-md"
													>
														<div class="mb-3 flex items-center justify-between">
															<h5 class="flex items-center text-sm font-semibold text-orange-800">
																<svg
																	class="mr-2 h-4 w-4"
																	fill="none"
																	stroke="currentColor"
															...
															<button
																...
																class="rounded-lg bg-orange-600 px-3 py-1.5 text-xs font-medium text-white shadow-sm transition-all duration-200 hover:bg-orange-700"
																type="button"
																title="Copy to clipboard"
															>
																{$t('letters.copy')}
	...
	{#if showLetterModal}
		...
		>
			<div
				class="mx-4 max-h-[90vh] w-full max-w-4xl overflow-hidden rounded-lg bg-white"
			...
			>
				<div class="flex items-center justify-between border-b border-gray-200 p-4">
					...
					<div class="flex items-center space-x-2">
						...
						{#if currentLetterId}
							<button
								type="button"
								class="rounded bg-green-600 px-3 py-2 text-sm text-white transition-colors hover:bg-green-700"
						...
						{#if letterVersions[currentLetterId] && letterVersions[currentLetterId].length > 1}
							<div class="flex flex-col items-start space-y-2">
								<select
									...
									class="rounded border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500"
									on:change={() => {
									...
									{#each letterVersions[currentLetterId] || [] as version}
										<option value={version}>
											{version.version_type === 'original'
												? $t('letters.original_version')
												: $t('letters.regenerated_version')} -
								...
								{#if selectedVersion[currentLetterId] && selectedVersion[currentLetterId].change_request_comment}
									<div class="mt-0.5 pl-3 text-xs text-blue-600">
										<button
						...
						{#if selectedVersion[currentLetterId] && selectedVersion[currentLetterId].version_type !== 'original' && letterVersions[currentLetterId] && letterVersions[currentLetterId].length > 1}
							<button
								on:click={() =>
									deleteLetterVersion(selectedVersion[currentLetterId].id, currentLetterId)}
						...
						{#if currentLetterContent && currentLetterContent.includes('pdf_url')}
							...
							{#await (async () => {
								try {
									const match = currentLetterContent.match(/pdf_url\s*[:=]\s*['"]([^'"]+)/);
								...
								{#if pdfUrl}
									<a
										...
										target="_blank"
										rel="noopener"
										class="rounded bg-indigo-600 px-3 py-2 text-sm text-white transition-colors hover:bg-indigo-700"
										aria-label="Download PDF">{$t('letters.download_pdf')}</a
									>
								{/if}
							{/await}
						{:else}
				...
				<div class="max-h-[calc(90vh-120px)] overflow-y-auto p-6">
					...
					{#if currentLetterId && showRegenerationSection}
						<div class="mb-6 border-b border-gray-200 bg-gray-50 px-6 py-4">
							<div>
								...
								<div class="flex items-end space-x-4">
									<!-- Target Tone -->
									<div>
										<label for="target-tone" class="mb-1 block text-xs text-gray-600"
											>{$t('letters.target_tone')}</label
										>
										<select
											id="target-tone"
									...
									<!-- Target Length -->
									<div>
										<label for="target-length" class="mb-1 block text-xs text-gray-600"
											>{$t('letters.target_length')}</label
										>
										<select
											id="target-length"
									...
									{#if !showChangeRequestField}
										<button
											...
											type="button"
											class="rounded border border-indigo-200 px-3 py-2 text-xs text-indigo-600 transition-colors hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-800"
										>
									...
									<button
										...
										class="rounded bg-indigo-600 px-4 py-2 text-sm text-white transition-colors hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50"
										type="button"
									...
									<div class="mt-4 border-t border-gray-200 pt-4">
										<div class="relative">
											...
											<button
												...
												type="button"
												class="absolute top-0 right-0 p-1 text-red-500 transition-colors hover:text-red-700"
											...
											<textarea
												...
												class="w-full resize-none rounded border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500"
												rows="2"
												maxlength="500"
											></textarea>
											<p class="mt-0.5 text-xs text-gray-500">
												{$t('letters.change_request_comment_hint')}
											</p>
										</div>
									</div>
								{/if}
			...
			>
				<div class="flex items-center justify-between border-b border-gray-200 p-4">
					<h3 id="change-instructions-title" class="text-lg font-semibold text-gray-900">
						...
						{#if selectedVersion[currentLetterId]}
							{selectedVersion[currentLetterId].version_type === 'original'
								? 'Original Version'
								: 'Regenerated Version'} -
				...
				<div class="max-h-[calc(80vh-120px)] overflow-y-auto p-6">
					{#if selectedVersion[currentLetterId] && selectedVersion[currentLetterId].change_request_comment}
						<div class="rounded-lg border border-blue-200 bg-blue-50 p-4">
							<h4 class="mb-3 text-sm font-medium text-blue-800">Change Request:</h4>
